...
- `float: left`/`float: right` siblings laid out side by side with exclusion regions that shorten following line boxes around the float; inline/replaced floats and `clear` are supported, while complete BFC interactions and complex block-content floats remain partial;
...
Still pending in block layout: complete BFC rules and broader reuse of intrinsic sizing across shrink-to-fit paths. Relative/absolute/fixed positioning exists, and floats now create exclusion regions so following text wraps around them; complex block-content float/BFC interactions remain partial. Table layout honors `colspan`, `rowspan`, captions, `vertical-align`, and `border-collapse: collapse`; column sizing now uses recursive min/max-content bounds, while per-column `<col>` hints and complete table header/footer fragmentation semantics remain open. Grid named areas are implemented; named grid lines are not.
