...
Still pending in pagination/paint: true isolated PDF transparency-group compositing (stacking order itself now flattens positioned descendants through non-context ancestors across the whole physical page, honors negative / normal-auto / non-negative numeric `z-index`, and isolates `opacity < 1` contexts), complete rounded asymmetric per-side borders, border styles beyond the current solid/dashed/dotted subset (`double`, `groove`, `ridge`, `inset`, `outset`), WebP/SVG paint and Adam7 PNG. 2D CSS transforms (`matrix`, translate, scale, rotate, skew and `transform-origin`) are implemented as paint scopes for blocks and atomic inline boxes; 3D/perspective and transformed link-hit geometry remain open. Element opacity and overflow clipping are implemented for the current paint model, but isolated opacity-group compositing and broader clipping semantics remain open.
