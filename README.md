...
Still pending in block layout: broader reuse of intrinsic sizing in flex/grid and the remaining edge cases of full CSS BFC semantics. Relative/absolute/fixed positioning exists; floats use scoped exclusions, side-specific clearance and multi-row placement. Table layout honors `colspan`, `rowspan`, captions, `vertical-align`, `border-collapse: collapse`, recursive min/max-content sizing and `<col>`/`<colgroup>` width hints (including spans); complete repeated table header/footer fragmentation semantics remain open. Grid named areas are implemented; named grid lines are not.
