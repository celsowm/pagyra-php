...
Still pending in pagination/paint: isolated opacity grouping for normal non-atomic inline spans (layout boxes and atomic inline boxes already use PDF Transparency Group Form XObjects; stacking order flattens positioned descendants across the physical page and honors negative / normal-auto / non-negative numeric `z-index`), complete rounded asymmetric per-side borders, border styles beyond the current solid/dashed/dotted subset (`double`, `groove`, `ridge`, `inset`, `outset`), WebP/SVG paint and Adam7 PNG. 2D CSS transforms (`matrix`, translate, scale, rotate, skew and `transform-origin`) are implemented as paint scopes for blocks and atomic inline boxes; 3D/perspective and transformed link-hit geometry remain open. Element opacity uses isolated PDF transparency groups for layout/atomic boxes, with the existing per-run fallback retained only for normal inline spans; broader clipping semantics remain open.
