...
- `display:flex` lays items out in rows or columns with wrapping, `order`, `gap`, `flex-grow`/`-shrink`/`-basis` (and the `flex` shorthand), `justify-content`, `align-items`/`align-self` (including `stretch`), `align-content` and main-axis auto margins; `display:grid` still falls back to plain block layout so content and its own nested layout are preserved;
