...
- `background` shorthand expansion for color/image/repeat/position/size, with URL backgrounds plus linear/radial gradients reaching the paint pipeline; attachment/origin/clip parsing remains a narrower subset;
...
- `list-item` markers for `<ol>`/`<ul>` (`list-style-position: outside`): `disc`/`circle`/`square` bullets and `decimal`, `decimal-leading-zero`, `lower`/`upper-alpha` (`-latin`) and `lower`/`upper-roman` numbering, resolved and formatted as in the reference's `list-utils`, painted in the list's left padding before the item's first line; the ordered counter is a sibling count that additionally honours `<ol start>` and `<li value>`, and `list-style-type:none` / `list-style:none` on the item or the list suppresses the marker;
...
- `float: left`/`float: right` siblings use BFC-scoped exclusion regions that shorten following line boxes; inline/replaced/block-content floats, side-specific `clear`, recursive shrink-to-fit and retry on the next float row when the current slot is full are supported;
- a table cell's declared `height` is a lower bound, not a fixed size, as CSS defines for table cells, so a cell whose content outgrows it still reports a box that covers that content instead of leaving it unclaimed by pagination;
...
- `display:flex` lays items out in rows or columns with wrapping, `order`, `gap`, `flex-grow`/`-shrink`/`-basis` (and the `flex` shorthand), `justify-content`, `align-items`/`align-self` (including `stretch`), `align-content` and main-axis auto margins; `display:grid` sizes px/%/`fr`/`auto`/`minmax()`/`repeat()` column tracks (with `auto-fill`), places items by line numbers and spans or auto-placement, sizes rows to content or `grid-template-rows`/`grid-auto-rows`, and honors `gap` and `align`/`justify-items`/`-self`; `grid-template-areas` / named areas are implemented, while named grid lines are not;
...
The optional `media` option (`'print'` by default, also `'screen'` or `'all'`) selects which `@media` blocks are live during the cascade, `@page` resolution and `@font-face` collection. A PDF is print, so `print` is the default; pass `media => 'screen'` to match wkhtmltopdf invoked without `--print-media-type`, e.g. for documents that ship `@media print { * { color: black !important } }` ("black prints faster") and would otherwise lose their link colors.

The optional `pagedBodyMargin` option (`'auto'` by default, or `'zero'`), as in `pagyra-js`, decides what happens to the body's margin. The cascade starts at `<html>` and `<body>`, so `body { ... }`, `html { font-size }` (which is what `rem` resolves against) and `<body class>` apply, and the body is laid out as a block box: with `auto` its margin is whatever the cascade gives it, the UA's 8px unless the document says otherwise; `zero` removes it unconditionally, so content starts at the page margin.
...
Still pending in block layout: broader reuse of intrinsic sizing in flex/grid and the remaining edge cases of full CSS BFC semantics. Relative/absolute/fixed positioning exists; floats use scoped exclusions, side-specific clearance and multi-row placement. Table layout honors `colspan`, `rowspan`, captions, `vertical-align`, `border-collapse: collapse`, recursive min/max-content sizing and `<col>`/`<colgroup>` width hints (including spans). `thead`/`tfoot` now repeat on each physical table fragment and consume real page space; tables whose body contains multi-row `rowspan` currently use the legacy non-repeating pagination path rather than duplicating a spanning cell incorrectly. Grid named areas are implemented; named grid lines are not.

Still pending in pagination/paint: isolated opacity grouping for normal non-atomic inline spans (layout boxes and atomic inline boxes already use PDF Transparency Group Form XObjects; stacking order flattens positioned descendants across the physical page and honors negative / normal-auto / non-negative numeric `z-index`), complete rounded asymmetric per-side borders, border styles beyond the current solid/dashed/dotted subset (`double`, `groove`, `ridge`, `inset`, `outset`), WebP paint and Adam7 PNG. Basic inline-SVG vector shapes now paint directly to PDF with viewBox/aspect-ratio mapping, transforms, fill/stroke and path curves; SVG defs/gradients/use/text/image/clipPath/mask/filter remain pending. 2D CSS transforms (`matrix`, translate, scale, rotate, skew and `transform-origin`) are implemented as paint scopes for blocks and atomic inline boxes; 3D/perspective and transformed link-hit geometry remain open. Element opacity uses isolated PDF transparency groups for layout/atomic boxes, with the existing per-run fallback retained only for normal inline spans; broader clipping semantics remain open.

Selectors cover type/class/id/attribute selectors, the four combinators, the structural pseudo-classes, `:not()`/`:is()`/`:where()`/`:root`, and `::before`/`::after` generated content. `::first-line` and `::first-letter` are implemented with documented partial semantics; `::marker`, the remaining shorthands/property parsers, complete Chromium-derived UA styles, richer media queries, richer `@font-face` descriptors and the remaining `pagyra-js` CSS surface are still pending.
...
See [PLAN.md](PLAN.md) for the porting roadmap and parity criteria, and [docs/SUPPORT_MATRIX.md](docs/SUPPORT_MATRIX.md) for the detailed live support/limitation matrix.
