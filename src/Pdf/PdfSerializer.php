...
final class PdfSerializer
{
    ...
    /**
     * Byte 0x80-0x9F overrides for the WinAnsi (CP1252) codepage; every other byte 0x00-0xFF
     * maps to the identical Unicode code point. Mirrors pagyra-js's WIN_ANSI_UNICODE table.
     */
    private const WIN_ANSI_OVERRIDES = [
    ...
    private static ?array $unicodeToWinAnsi = null;

    /**
     * @return array<int,int> Unicode code point => WinAnsi byte, built once and cached.
     */
    private static function unicodeToWinAnsiMap(): array
    {
        if (self::$unicodeToWinAnsi !== null) return self::$unicodeToWinAnsi;
        $map = [];
        for ($byte = 0x00; $byte <= 0xFF; $byte++) {
            $unicode = self::WIN_ANSI_OVERRIDES[$byte] ?? $byte;
            $map[$unicode] ??= $byte;
        }
        return self::$unicodeToWinAnsi = $map;
    }
